...
?>
    ...
    <body>
        ...
        if(isset($_GET['rech'])){
            ...
            if ($t_result==0) {
                echo ('<h2>Aucune villes trouvées</h2>');
            } elseif ($t_result==1) { 
                ...
                echo '<h3>Resultat:</h3>';
                echo ('<h3>'.$oneresultat['nom'].'</h3>');
                echo ('<strong>Region: '.$oneresultat['nom-reg'].'</strong><br/>');
                echo ('<h4>Departement: '.$oneresultat['nom-dpt'].'</h4>');

                
            }  elseif ($t_result>1) {
                echo ('<h4>Plusieurs villes correspondent à votre recherche<br/> Veuillez préciser votre demande:</h4><br>');
              /*  echo ('<pre>');
                print_r ($resultat);
                echo ('</pre>');
                */
                 foreach($resultat as $iresult) {
                    ...
                    echo ('<input type="submit" name="choixville">'.$iresult['nom'].'</input>&nbsp;');
                    echo ('Region: '.$iresult['nom-reg'].'<br/>');
                    echo ('</form>');}
                if(isset($_POST['choixville'])) {
                    echo'<p>'.$iresult['nom'].'</p>';
                    echo'<p>'.$iresult['nom-dpt'].'</p>';
                    echo'<p>'.$iresult['nom-reg'].'</p>';
                    }
                 }
            }
